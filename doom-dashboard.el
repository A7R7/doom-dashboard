...
(defvar doom-dashboard-banner-file "default.png"
 ...
 If nil, always use the ASCII banner.")
...
(defvar doom-dashboard-pwd-policy "~" ;; FIXME
...
(defvar doom-dashboard--width 120)
...
(defvar doom-init-time nil
  "The time it took, in seconds, for Doom Emacs to initialize.") ;; FIXME
...
(defvar doom-dashboard-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "<left-margin> <mouse-1>")       'ignore)
    (define-key map (kbd "SPC")                           'ignore)
    (define-key map (kbd "n")                             'forward-button)
    (define-key map (kbd "p")                             'backward-button)
    (define-key map (kbd "C-n")                           'forward-button)
    (define-key map (kbd "C-p")                           'backward-button)
    (define-key map (kbd "<up>")                          'forward-button)
    (define-key map (kbd "<down>")                        'backward-button)
    (define-key map (kbd "<tab>")                         'forward-button)
    (define-key map (kbd "<backtab>")                     'backward-button)
    (keymap-set map "<remap> <evil-next-line>"            'forward-button)
    (keymap-set map "<remap> <evil-previous-line>"        'backward-button)
    (keymap-set map "<remap> <evil-next-visual-line>"     'forward-button)
    (keymap-set map "<remap> <evil-previous-visual-line>" 'backward-button)
    (keymap-set map "<remap> <evil-paste-pop-next>"       'forward-button)
    (keymap-set map "<remap> <evil-paste-pop>"            'backward-button)
    (keymap-set map "<remap> <evil-delete>"               'ignore)
    (keymap-set map "<remap> <evil-delete-line>"          'ignore)
    (keymap-set map "<remap> <evil-insert>"               'ignore)
    (keymap-set map "<remap> <evil-append>"               'ignore)
    (keymap-set map "<remap> <evil-replace>"              'ignore)
    (keymap-set map "<remap> <evil-enter-replace-state>"  'ignore)
    (keymap-set map "<remap> <evil-change>"               'ignore)
    (keymap-set map "<remap> <evil-change-line>"          'ignore)
    (keymap-set map "<remap> <evil-visual-char>"          'ignore)
    (keymap-set map "<remap> <evil-visual-line>"          'ignore)
    (keymap-set map "<remap> <meow-next>"                 'forward-button)
    (keymap-set map "<remap> <meow-prev>"                 'backward-button)
		map)
	"Keymap for doom-dashboard mode")

;;
;;; Major mode

(define-derived-mode doom-dashboard-mode special-mode "Doom Dashboard"
  ...
  ;; activate it there (by pretending it's already active). ;; FIXME: comment
  (hl-line-mode nil))
...
'real' means. ;; FIXME: solaire-mode-real-buffer
...
What it is set to is controlled by `doom-dashboard-pwd-policy'."
  (if pwd
      (with-current-buffer (doom-fallback-buffer)
        (message "Changed dashboard's PWD to %s" pwd) ;; change doom-log to message
...
(defun doom-dashboard-reload (&optional force)
  ...
  (when (or (and (not doom-dashboard-inhibit-refresh)
            ...
            force)
    (with-current-buffer (doom-fallback-buffer)
      (message "Reloading dashboard at %s" (format-time-string "%T")) ;; change doom-log to message
      (with-silent-modifications
        (let ((pt (point)))
...
(defun doom-dashboard-widget-footer ()
  (insert
   ...
   (doom-dashboard--center
    ...
    (with-temp-buffer
      (insert-text-button (or (nerd-icons-codicon "nf-cod-github" :face 'doom-dashboard-footer-icon :height 1.3 :v-adjust -0.15)
